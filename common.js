...
const POD_BOOKING = (function () {
  ...
  const DEFAULT_COLS = { user: 0, client: 1, clientId: 2, pod: 3, sensitivity: 4 };
  ...
  function normalizePod(value) {
    return normalizeText(value).replace(/^pod[\s\-_]*/i, "").replace(/[\s\-_]/g, "");
  }
  ...
  function detectColumns(rows) {
    ...
    const cols = hasHeader ? {
      ...
      clientId: find(["client id", "clientid", "client_id", "client code", "clientcode"], DEFAULT_COLS.clientId),
  ...
  async function getClientListWithPOD() {
    return Excel.run(async (ctx) => {
      ...
      for (let i = startRow; i < rows.length; i++) {
        ...
        if (c && !seen.has(c)) { seen.add(c); out.push({ client: c, pod: p }); }
      }
      return out;
    });
  }
   ...
   *
   * This function MUST run inside the master workbook. The `New Client` task
   * pane is delivered only by manifest-masterdb.xml, but as a defensive
  ...
  async function saveNewClient(payload) {
    return Excel.run(async (ctx) => {
      ...
      if (table.isNullObject) {
        throw new Error(
          "Master Database table '" + DB_TABLE_NAME +
      ...
      const aliases = {
        ...
        clientId:    ["client id", "clientid", "client code", "clientcode", "client_id"],
        pod:         ["pod"],
      ...
      header.forEach((h, i) => {
        ...
        for (const key of Object.keys(aliases)) {
          if (aliases[key].includes(n) && idx[key] === undefined) idx[key] = i;
        }
      });
      ...
      return { ok: true, rowIndex: added.index, columns: header };
    });
  }
  ...
  async function getMasterTableHeader() {
    return Excel.run(async (ctx) => {
      ...
      headerRange.load("values");
      await ctx.sync();
      return (headerRange.values && headerRange.values[0]) || [];
    });
  }
  ...
  async function appendTime(timeValue) {
    return Excel.run(async (ctx) => {
      ...
      const old = String((cell.values && cell.values[0] && cell.values[0][0]) || "");
  ...
  async function saveDateTime(day, hour, minute, ampm) {
    return Excel.run(async (ctx) => {
      const cell = ctx.workbook.getActiveCell();
      const formatted = day.substring(0, 3) + ", " + hour + ":" + String(minute).padStart(2, "0") + " " + ampm;
